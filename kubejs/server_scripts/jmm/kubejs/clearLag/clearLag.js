            ...
            server.schedule(MINUTE * 4, () => {
                if (runCL) {
                    ...
                    server.schedule(SECOND * 50, () => {
                        if (runCL) {
                            server.tell([Text.lightPurple('[ClearLag]'), ' Removing all items on ground in 10 seconds!'])
                            server.schedule(SECOND * 7, () => {
                                if (runCL) {
                                    server.tell([Text.lightPurple('[ClearLag]'), ' Removing all items on ground in 3 seconds!'])
                                    server.schedule(SECOND * 1, () => {
                                        if (runCL) {
                                            server.tell([Text.lightPurple('[ClearLag]'), ' Removing all items on ground in 2 seconds!'])
                                            server.schedule(SECOND * 1, () => {
                                                if (runCL) {
                                                    server.tell([Text.lightPurple('[ClearLag]'), ' Removing all items on ground in 1 second!'])
